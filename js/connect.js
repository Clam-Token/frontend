...
async function fetchAccountData() {
    ...
    addressClam = contract.Clam.address;
    addressMarket = contract.Market.address;
    addressLottery = contract.Lottery.address;
    if (chainId == TESTNET) addressClam = contract.Clam.address_test;
    ...
    clam = new web3.eth.Contract(contract.Clam.abi, addressClam);
    ...
    clam.events.Approval(function(error, result) {
        if (!error) {
    ...
    clam.events.Transfer(function(error, result) {
        if (!error) {
    ...
    clam.events.Claimed(function(error, result) {
        if (!error) {
            reloadClam();
        } else
            console.log(error);
